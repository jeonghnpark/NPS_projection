...
plt.plot(
    demographic_df["year"],
    demographic_df["total_population"] / 1_000_000,
    marker="o",
    label="총인구",
)
plt.plot(
    demographic_df["year"],
    demographic_df["working_age_population"] / 1_000_000,
    marker="o",
    label="생산가능인구",
)
plt.title("인구 추이", fontsize=12)
plt.xlabel("연도", fontsize=10)
plt.ylabel("인구 (백만명)", fontsize=10)
plt.legend()
plt.grid(True)
...
plt.plot(
    demographic_df["year"],
    demographic_df["elderly_population"] / 1_000_000,
    marker="o",
    color="red",
)
plt.title("노인인구 추이", fontsize=12)
plt.xlabel("연도", fontsize=10)
plt.ylabel("인구 (백만명)", fontsize=10)
plt.grid(True)
...
plt.title("노년부양비 추이", fontsize=12)
plt.xlabel("연도", fontsize=10)
...
line1 = ax1.plot(
    demographic_df["year"],
    demographic_df["total_subscribers"] / 1_000_000,
    marker="o",
    color="blue",
    label="가입자 수",
)
line2 = ax2.plot(
    demographic_df["year"],
    demographic_df["total_income_real"] / 1_000_000_000_000,
    marker="o",
    color="purple",
    label="실질소득",
)

ax1.set_ylabel("가입자 수 (백만명)", color="blue", fontsize=10)
ax2.set_ylabel("실질소득 (조원)", color="purple", fontsize=10)

lines = line1 + line2
labels = [l.get_label() for l in lines]
ax1.legend(lines, labels, loc="upper right")

plt.title("가입자 수 및 실질소득 추이", fontsize=12)
plt.xlabel("연도", fontsize=10)
plt.grid(True)

plt.tight_layout()
plt.savefig(f"demographic_projection_{timestamp}.png", dpi=300, bbox_inches="tight")
plt.close()

print(
    f"인구추계 시각화 결과가 demographic_projection_{timestamp}.png로 저장되었습니다."
)
